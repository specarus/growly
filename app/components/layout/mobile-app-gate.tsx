...
const AppLink = ({
  ...
  <a
    href={href}
    className={`inline-flex items-center justify-center rounded-full border border-white/40 bg-primary text-white px-4 md:px-6 py-3 md:py-4 text-center text-base md:text-xl font-semibold shadow-sm shadow-black/15 transition hover:-translate-y-0.5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary ${className}`}
    target="_blank"
    rel="noreferrer"
  >
    {label}
      ...
      <div className="relative mx-auto flex w-full flex-col gap-8 md:gap-10 py-10 md:py-16 px-6 md:px-8">
        <div className="flex flex-col items-center gap-4 md:gap-6">
          <div className="flex items-center justify-center gap-4 py-4">
            <Sprout className="w-6 h-6 md:w-10 md:h-10 text-green-soft" />
            <p className="text-sm md:text-xl uppercase tracking-[0.6em] text-muted-foreground">
              Growly
            </p>
          </div>
        ...
        <div className="flex flex-col items-stretch gap-4 md:gap-6 rounded-3xl md:rounded-4xl border border-white/70 bg-white/80 p-4 md:p-6 shadow-inner backdrop-blur-lg">
          <div className="flex flex-1 flex-col gap-4 md:gap-6 text-foreground">
            <div className="flex flex-col gap-3 md:gap-4">
              <div className="flex flex-col gap-1">
                <p className="text-[11px] md:text-sm uppercase tracking-[0.35em] text-muted-foreground">
                  Tap into the app
                </p>
              </div>
          ...
          <div className="flex-1 space-y-4 rounded-2xl border border-white/50 bg-linear-to-b from-white to-muted/30 p-4 md:p-6 shadow-inner shadow-black/10">
            ...
            <div className="space-y-3">
              {featureTiles.map((feature) => (
                <div
                  key={feature.title}
                  className="rounded-2xl border border-muted/40 bg-white/70 p-3 md:p-4"
                >
